...
export function EditScoreModal({ t, lang, entry, score, setScore, onSave, onCancel }: Props) {
  ...
  return (
    <Modal>
      ...
      <div style={{ fontSize: 13, color: "var(--muted-foreground)", marginBottom: 12 }}>{lang === "bn" ? entry.questionText : entry.questionTextEn} · max {max}</div>
      ...
      <div style={S.fg}><label style={S.lbl}>{lang === "bn" ? "নতুন পয়েন্ট" : "New Score"} (max:{max})</label><input style={{ ...S.inp, maxWidth: 120, fontSize: 18, fontWeight: 700, color: "var(--foreground)" }} type="number" min="0" max={max} value={score} onChange={(e) => setScore(Math.min(parseInt(e.target.value) || 0, max))} /></div>
