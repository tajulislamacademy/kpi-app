              ...
              {filterDefs.map(({ l, v, set, opts, searchable }) => (
                <div key={l} className="space-y-1"><Label className="text-xs">{l}</Label>
                  {searchable
                    ...
                    : <Select value={v} onValueChange={set}><SelectTrigger className="h-9 w-full"><SelectValue /></SelectTrigger><SelectContent>{opts.map(o => <SelectItem key={o.v} value={o.v}>{o.l}</SelectItem>)}</SelectContent></Select>}
                </div>
              ))}
