...
export function SettingsPage({ t, lang, termConfig, onSaveTermConfig, showNotif }: Props) {
  ...
  return (<div style={S.page}><h2 style={S.pt}>{t.settings}</h2>
    ...
    <div style={S.card}><h3 style={S.ct}>{t.termConfig}</h3>
      {TERMS.map((term, ti) => (<div key={term} style={{ marginBottom: 20 }}><div style={{ fontWeight: 700, color: "#0f172a", fontSize: 14, marginBottom: 8 }}>{ti === 0 ? t.term1 : ti === 1 ? t.term2 : ti === 2 ? t.term3 : t.term4}</div><MonthsPicker lang={lang} value={cfg[term]} onToggle={(mi) => toggle(term, mi)} /></div>))}
    ...
    <div style={S.card}><h3 style={S.ct}>{lang === "bn" ? "বর্তমান কনফিগারেশন" : "Current Configuration"}</h3>
      {TERMS.map((term, ti) => (<div key={term} style={{ padding: "8px 0", borderBottom: "1px solid #f1f5f9", fontSize: 14, color: "#374151" }}><strong>{ti === 0 ? t.term1 : ti === 1 ? t.term2 : ti === 2 ? t.term3 : t.term4}:</strong><span style={{ marginLeft: 8 }}>{termConfig[term].map(m => T[lang][MONTHS[m]]).join(", ") || "—"}</span></div>))}
    </div>
  </div>);
}
