  ...
  return (<div style={S.page}>
    ...
    {viewQ && (<Modal maxWidth={520}>
      ...
      <div style={S.fg}><label style={{ ...S.lbl, marginBottom: 8 }}>{t.activeMonths}:</label>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{MONTHS.map((m, mi) => (<span key={m} style={{ padding: "3px 9px", borderRadius: 6, fontSize: 12, fontWeight: 600, background: viewQ.activeMonths.includes(mi) ? "var(--primary)" : "var(--muted)", color: viewQ.activeMonths.includes(mi) ? "#fff" : "var(--muted-foreground)" }}>{T[lang][m]}</span>))}</div>
      </div>
