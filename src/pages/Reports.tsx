...
export function ReportsPage({ t, lang, termConfig, currentUser, isAdmin, selectedYear, setSelectedYear }: Props) {
  ...
  return (<div style={S.page}>
    ...
    {!isStudent && !isParent && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 12, margin: "16px 0" }}>{ranked.slice(0, 3).map((s, i) => (<div key={s.id} style={{ ...S.card, textAlign: "center", border: "1px solid var(--border)" }}><div style={{ fontSize: 28 }}>{i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉"}</div><div style={{ fontWeight: 700, fontSize: 14, color: "var(--foreground)" }}>{lang === "bn" ? s.name : s.nameEn}</div><div style={{ fontSize: 12, color: "var(--muted-foreground)" }}>{t.class} {s.class}{s.section}</div><div style={{ fontSize: 26, fontWeight: 900, color: "var(--foreground)" }}>{s.kpi}</div><div style={{ fontSize: 11, color: "var(--muted-foreground)" }}>{lang === "bn" ? "পয়েন্ট" : "pts"}</div></div>))}</div>}
    <div style={S.card}><h3 style={S.ct}>{lang === "bn" ? "র‍্যাংকিং" : "Rankings"}</h3>
      ...
      <tbody>{ranked.map((s, i) => { const isMe = isStudent && s.id === currentUser.id; return (<tr key={s.id} style={{ ...(i % 2 === 0 ? { background: "var(--muted)" } : {}), fontWeight: i < 3 ? 700 : 400, ...(isMe ? { background: "var(--muted)" } : {}) }}><td style={S.td}><span style={{ display: "inline-block", width: 28, height: 28, lineHeight: "28px", textAlign: "center", borderRadius: 6, background: i === 0 ? "#fef3c7" : i === 1 ? "#f1f5f9" : i === 2 ? "#fff7ed" : "transparent", color: i < 3 ? "#0f172a" : "var(--muted-foreground)", fontWeight: 700 }}>{i < 3 ? ["🥇", "🥈", "🥉"][i] : i + 1}</span></td><td style={S.td}>{lang === "bn" ? s.name : s.nameEn}{isMe && <span style={{ fontSize: 11, color: "var(--foreground)", marginLeft: 6 }}>(আমি)</span>}</td><td style={S.td}>{s.class}{s.section}</td><td style={S.td}>{s.roll}</td><td style={S.td}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ height: 8, borderRadius: 4, background: "linear-gradient(90deg, var(--primary), var(--muted-foreground))", width: `${Math.min(100, (s.kpi / (ranked[0]?.kpi || 1)) * 100)}%`, minWidth: 4 }} /><span style={{ fontSize: 13, fontWeight: 700, color: "var(--foreground)" }}>{s.kpi}</span></div></td></tr>); })}</tbody></table></div>
    </div>
  </div>);
}
